...
function main() {
    ...
    const currentBranch = execCommand('git rev-parse --abbrev-ref HEAD', 'Checking current branch');
    if (currentBranch !== 'main') {
        log(`⚠️  Warning: You're on branch '${currentBranch}', not 'main'`, 'yellow');
    ...
    const versionResult = execCommand(`npm version ${releaseType} --no-git-tag-version`, `Updating ${releaseType} version`);
    ...
    log('📦 Check releases at: https://github.com/raulduk3/layer7-ai-voice/releases', 'blue');
}
